...
class GoogleMap extends Component {
   ...
   *      None
   */
  static populateInfoWindow = (map, marker, infowindow, location) => {
    ...
    if (infowindow.getPosition() !== marker.position) {
      ...
      infowindow.setContent(`<div role="dialog" class="infowindow" tabindex="0" aria-labelledby="infowindow-help">
      <h2>${marker.title}</h2>
      <p id="infowindow-help">Info</p>
  ...
  animateSelectedPlaceOnMap = (placeTitle) => {
    ...
    for (let i = 0; i < markers.length; i += 1) {
      if (placeTitle === markers[i].title) {
        ...
        GoogleMap.populateInfoWindow(map, markers[i], largeInfowindow, locations[i]);
      } else {
  ...
  loadMarkersOnMap = () => {
    ...
    for (let i = 0; i < locations.length; i += 1) {
      ...
      marker.addListener('click', () => (function captureMarker() {
        GoogleMap.populateInfoWindow(map, marker, largeInfowindow, locations[i]);
      }(marker)));
